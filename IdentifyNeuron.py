...
def LoadNeuronState(datatypelist = ['chosen', 'rejected']):
    ...
    numSample = torch.tensor(numSample)
    over_zero = torch.stack(over_zero, dim=-1)

    return numSample, over_zero

n_samples, over_zero = LoadNeuronState()
num_layers, intermediate_size, lang_num = over_zero.size()

def activation(outputfile):    
    top_rate = 0.01
    filter_rate = 0.95
    activation_bar_ratio = 0.95
    activation_probs = over_zero / n_samples # layer x inter x lang_num
